...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:variable name="version">
    <xsl:text>1.0 beta for MerMEId</xsl:text>
  </xsl:variable>
  ...
  <xsl:template match="mei:annot[mei:lg or mei:p or mei:quote or mei:table]" mode="copy">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test="mei:p">
          <xsl:copy>
            <xsl:apply-templates select="@* | node()" mode="copy"></xsl:apply-templates>
          </xsl:copy>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="mei:instrumentation" mode="copy">
    <perfResList xmlns:mei="http://www.music-encoding.org/ns/mei"
      xsl:exclude-result-prefixes="mei
      xlink">
      <xsl:apply-templates select="@*[not(name()='xml:id') or not(name()='mei:n')]" mode="copy"/>
      <xsl:if test="@xml:id">
        <xsl:attribute name="xml:id">
          <xsl:value-of select="concat('perfResList', substring-after( @xml:id, 'instrumentation'))"/>
        </xsl:attribute>
      </xsl:if>
      <xsl:if test="@n">
        <xsl:attribute name="n">
          <xsl:value-of select="concat('perfResList', substring-after( @n, 'instrumentation'))"/>
        </xsl:attribute>
      </xsl:if>
  ...
  <xsl:template match="mei:instrVoiceGrp" mode="copy">
    <perfResList xmlns:mei="http://www.music-encoding.org/ns/mei"
      ...
      xlink">
      <xsl:apply-templates select="@*[not(name()='xml:id') or not(name()='mei:n')]" mode="copy"/>
      <!-- Copies id, and renames prefix -->
      <xsl:if test="@xml:id">
        <xsl:attribute name="xml:id">
          <xsl:value-of select="concat('perfResList', substring-after( @xml:id, 'instrVoiceGrp'))"/>
        </xsl:attribute>
      </xsl:if>
      <!-- Same with n-reference -->
      <xsl:if test="@n">
        <xsl:attribute name="n">
          <xsl:value-of select="concat('perfResList', substring-after( @n, 'instrVoiceGrp'))"/>
        </xsl:attribute>
      </xsl:if>
  ...
  <xsl:template match="mei:instrVoice | mei:ensemble" mode="copy">
    <perfRes xmlns:mei="http://www.music-encoding.org/ns/mei"
      ...
      <xsl:if test="@xml:id">
        <xsl:attribute name="xml:id">
          <xsl:value-of select="concat('perfRes', substring-after( @xml:id, 'instrVoice'))"/>
        </xsl:attribute>
      </xsl:if>
      <xsl:if test="@n">
        <xsl:attribute name="n">
          <xsl:value-of select="concat('perfRes', substring-after( n, 'instrVoice'))"/>
        </xsl:attribute>
      </xsl:if>
      <xsl:value-of select="."/>
    </perfRes>
    ...
    <xsl:copy>
      <xsl:choose>
        ...
        <xsl:when
          ...
          <xsl:if test="mei:p">
            <!-- if event has describing <p>, rename it to <desc> -->
            <desc xmlns:mei="http://www.music-encoding.org/ns/mei"
              xsl:exclude-result-prefixes="mei xlink">
              <xsl:if test="mei:p[@xml:id]">
                <xsl:attribute name="xml:id">
                  <xsl:value-of select="concat('desc', substring-after( mei:p/@xml:id, 'p'))"/>
                </xsl:attribute>
              </xsl:if>
              <xsl:copy-of select="mei:p/node()"/>
            </desc>
          </xsl:if>
          
          <!-- Insert biblist after desc element -->
          <xsl:apply-templates
            select="mei:biblList"
            mode="copy"/>
        </xsl:when>
  ...
  <!-- remove acqSource elements -->
  <xsl:template match="mei:acqSource" mode="copy"/>
  
  <!-- remove empty castList elements -->
  <xsl:template match="mei:castList[not(*)]" mode="copy"/>  

  <xsl:template match="mei:change" mode="copy">
    <change xmlns:mei="http://www.music-encoding.org/ns/mei"
      xsl:exclude-result-prefixes="mei xlink">
      <!-- move value in @resp til respStmt -->
      <xsl:apply-templates select="@*[local-name()!='resp']" mode="copy"/>
      <respStmt>
        <xsl:apply-templates select="mei:respStmt/@*" mode="copy"/>
        <resp><xsl:value-of select="@resp"/></resp>
        <!-- in case there are elements inside <respStmt> already: -->
        <xsl:apply-templates select="mei:respStmt/*" mode="copy"/>
      </respStmt>
      <xsl:apply-templates select="*[local-name()!='respStmt']" mode="copy"/>
    </change>
  </xsl:template>
