...
declare function app:format-reference(
  ...
  <tr class="{$class}">
    <td>
      {$doc/m:meihead/m:filedesc/m:titlestmt/m:respstmt/m:persname[@type='composer']}
    </td>
...
declare function app:get-edition-and-number($doc as node() ) as xs:string* {
  let $c := 
    $doc//m:meihead/m:encodingdesc/m:projectdesc/m:p/m:list[@n='use']/m:item/string()
  return ($c,$doc//m:pubstmt/m:identifier[@type=$c]/string())
};
...
declare function app:edit-form-reference($doc as node()) as node() {
  ...
  let $ref := 
      <a  title="edit" 
          href="/orbeon/xforms-jsp/mei-form/?http://{request:get-header('HOST')}/form/dcm/{util:document-name($doc)}">
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <form action="" method="get" class="search">
    <fieldset>
        ...
        <input type="submit" value="Clear" onclick="this.form.query.value=''; return true;"/>
  ...
  <p>
	...
	collection("/db/dcm")/m:mei/m:meihead/m:encodingdesc/m:projectdesc/m:p/m:list[@n='use']/m:item/string()[string-length(.) > 0])
	  let $querystring  := 
	    if($query) then
	      fn:string-join(
		  ("c=",
    ...
    }
    {
      ...
      let $request := 
	if($query) then
	  fn:string-join(("list_files.xq?query=",fn:escape-uri($query,true())),"")
        else
	  "list_files.xq"

      let $link := 
	if($coll) then 
	  <button type="submit" title="All collections" onclick="location.href='?{$request}'; return false;">All collections</button>
	else
