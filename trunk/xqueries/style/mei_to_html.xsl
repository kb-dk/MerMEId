<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:variable name="bibl_file_name" select="string(concat($settings/dcm:parameters/dcm:server_name,$settings/dcm:parameters/dcm:exist_dir,'library/standard_bibliography.xml'))"/>
	<xsl:variable name="bibl_file" select="document($bibl_file_name)"/>
	<xsl:variable name="abbreviations_file_name" select="string(concat($settings/dcm:parameters/dcm:server_name,$settings/dcm:parameters/dcm:exist_dir,'library/abbreviations.xml'))"/>
	<xsl:variable name="abbreviations" select="document($abbreviations_file_name)/m:p/*"/>
	
	<xsl:variable name="language_pack_file_name">
		<xsl:choose>
			<xsl:when test="$language!=''">
                <xsl:value-of select="string(concat($settings/dcm:parameters/dcm:server_name,$settings/dcm:parameters/dcm:exist_dir,'style/language/',$language,'.xml'))"/>
            </xsl:when>
			<xsl:otherwise>
                <xsl:value-of select="string(concat($settings/dcm:parameters/dcm:server_name,$settings/dcm:parameters/dcm:exist_dir,'style/language/',$default_language,'.xml'))"/>
            </xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:template match="*" mode="settings_menu">
		<div class="settings colophon noprint">
			<a href="javascript:loadcssfile('/editor/style/html_hide_languages.css'); hide('load_alt_lang_css'); show('remove_alt_lang_css')" id="load_alt_lang_css" class="noprint">
                <xsl:value-of select="$l/hide_alt_lang"/>
            </a>
			<a style="display:none" href="javascript:removecssfile('/editor/style/html_hide_languages.css'); hide('remove_alt_lang_css'); show('load_alt_lang_css')" id="remove_alt_lang_css" class="noprint">
                <xsl:value-of select="$l/show_alt_lang"/>
            </a>
		</div>
	</xsl:template>
	...
	<xsl:template match="m:relation" mode="relation_link">
		...
		<xsl:if test="$mermeid_crossref='true'">
			...
			<xsl:choose>
				<xsl:when test="doc-available((string(concat($settings/dcm:parameters/dcm:server_name,$settings/dcm:parameters/dcm:document_root,@target))))">
        			<xsl:variable name="fileName" select="concat($settings/dcm:parameters/dcm:server_name,$settings/dcm:parameters/dcm:document_root,@target)"/>
			    ...
			    <xsl:otherwise>
			    	<xsl:element name="img">
			    		<xsl:attribute name="src">/editor/images/warning.png</xsl:attribute>
			    		<xsl:attribute name="title">File <xsl:value-of select="@target"/> not found</xsl:attribute>
			    	</xsl:element>
			    </xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	...
	<xsl:template match="*[@auth.uri!='' and @codedval!=''][not(name()='perfResList' or name()='perfRes')]">
		...
		<xsl:if test="$display_authority_links = 'true'">
			<xsl:element name="a">
				...
				<xsl:attribute name="style">text-decoration:none;</xsl:attribute>
				<img src="/editor/images/external_link.gif" alt="link" title="Link to authority file" border="0"/>
            </xsl:element>
		</xsl:if>
	</xsl:template>
		...
		<div class="fold">
			<h3 class="section_heading" id="p{$id}">
				<span onclick="toggle('{$id}')" title="Click to show or hide">
					<img class="noprint" id="img{$id}" border="0" src="/editor/images/plus.png" alt="+"/>
	...
	<xsl:template match="m:identifier[@auth='RISM' or @auth='rism'][text()]">
		...
		<xsl:choose>
			...
			<xsl:when test="string-length($country)&gt;0 and     string-length($archive)&gt;0 and     string-length(translate($country,$vUpper,''))=0 and      string-length(translate($archive,$vAlpha,''))=0">
				<xsl:variable name="RISM_file_name" select="string(concat($settings/dcm:parameters/dcm:server_name,$settings/dcm:parameters/dcm:exist_dir,'rism_sigla/',      substring-before(normalize-space(.),'-'),'.xml'))"/>
				<xsl:choose>
